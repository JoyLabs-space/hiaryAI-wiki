      ...
      function: {
        ...
        parameters: {
          ...
          properties: {
            location: {
              type: 'string',
              description: '都市名と州、例：Tokyo, Japan'
            },
            unit: {
              type: 'string', 
              enum: ['celsius', 'fahrenheit'],
              description: '使用する温度単位'
            }
          },
          required: ['location']
        }
...
const result = await weatherAgent.run('東京の天気はどうですか？');`;

export const toolsHostedToolsExample = `import { Agent } from '@openai/agents';

const researchAgent = new Agent({
  model: 'gpt-4o-mini',
  instructions: 'ウェブ検索を使用してトピックの調査を支援します。',
  tools: [
    {
      type: 'browser', // ウェブブラウジング用ホステッドツール
      browser: {
        enabled: true
      }
    },
    {
      type: 'code_interpreter', // コード実行用ホステッドツール
      code_interpreter: {
        enabled: true
      }
    }
...
const result = await researchAgent.run('AIの最新トレンドについて調査してください');`;

export const nonStrictSchemaTools = `import { Agent } from '@openai/agents';

const flexibleAgent = new Agent({
  model: 'gpt-4o-mini',
  instructions: '柔軟なデータ構造で作業できます。',
  tools: [
    {
      type: 'function',
      function: {
        name: 'process_data',
        description: '柔軟なデータ構造を処理',
        parameters: {
          type: 'object',
          properties: {
            data: {
              type: 'object',
              additionalProperties: true, // 柔軟なプロパティを許可
              description: '任意のデータ構造'
            }
          },
          required: ['data']
        },
        strict: false // 厳密でないスキーマ検証
      }
    }
  ]
});`;
...
const mathAgent = new Agent({
  model: 'gpt-4o-mini',
  instructions: '数学の専門家です。数学的問題を解決します。',
});

const scienceAgent = new Agent({
  model: 'gpt-4o-mini',
  instructions: '科学の専門家です。科学的質問に答えます。',
});

const mainAgent = new Agent({
  model: 'gpt-4o-mini',
  instructions: '質問を適切な専門家にルーティングします。',
  tools: [
    {
      type: 'function',
      function: {
        name: 'ask_math_expert',
        description: '数学専門家に質問する',
        parameters: {
          type: 'object',
          properties: {
            question: { type: 'string', description: '数学の質問' }
          },
          required: ['question']
        }
      }
...
const agent = new Agent({
  model: 'gpt-4o-mini',
  instructions: 'MCPを通じてローカルツールにアクセスできます。',
  tools: [
    {
      type: 'mcp',
      mcp: {
...
const result = await agent.run('ローカルファイルシステムツールを使用してください');`;
