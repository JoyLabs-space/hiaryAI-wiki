...
export const localContextExample = `import { Agent, RunContext } from '@openai/agents';
...
const agent = new Agent({
  model: 'gpt-4o-mini',
  instructions: 'Access user context to provide personalized responses.',
  tools: [{
    type: 'function',
    function: {
...
const getUserPreferences = (context: RunContext<MyContext>) => {
  return {
    theme: context.data.preferences.theme,
    language: context.data.preferences.language
  };
};
...
const result = await agent.run('What are my current preferences?', { context });`;
