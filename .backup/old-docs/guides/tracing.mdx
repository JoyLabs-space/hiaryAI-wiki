...
export const customTraceExample = `import { Agent, Runner, trace } from '@openai/agents';
...
const result = await runner.run('Hello!', {
  onTrace: (traceEvent) => {
    console.log('Trace event:', traceEvent.type, traceEvent.data);
  }
});

// You can also add custom trace events manually
trace.addEvent({
  type: 'custom',
  data: {
...
export const cloudflareWorkers = `import { Agent, Runner, setTracingConfig } from '@openai/agents';

// Configure tracing for Cloudflare Workers environment
setTracingConfig({
  endpoint: 'https://api.openai.com/v1/traces',
  apiKey: env.OPENAI_API_KEY, // Access environment variable in Workers
  batchSize: 10,
  flushInterval: 5000, // 5 seconds
  // Disable automatic flushing for serverless environments
  autoFlush: false
});
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      const result = await runner.run('Process this request');
      
      // Manually flush traces before response
      await trace.flush();
      ...
      // Ensure traces are flushed even on errors
      await trace.flush();
      throw error;
    }
  }
