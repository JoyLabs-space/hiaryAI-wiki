...
// Using the utility run() function
...
try {
  const result = await runner.run('Perform a risky operation');
...
const agent = new Agent({
  model: 'gpt-4o-mini',
  instructions: 'You are a helpful conversational assistant. Keep track of our conversation.',
});
...
async function startChatLoop() {
  ...
  while (true) {
    // Get user input (in a real app, this would come from stdin or UI)
...
async function getUserInput(): Promise<string> {
  // In a real application, use readline or your UI framework
